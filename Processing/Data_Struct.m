...
% First Step: Save processed struct for analysising 
...
for People=1:30
    for TDVR=1:2   %2D=1; VR=2;
       ...
       %% 3.2. ECG Features: Rate: Mean/Std; RR: SDNN/SDSD
        RESULT{People}{TDVR}{videoChan}{12}=[];  % Be careful
        for event=1:videos
            ...
            for i=1:tenSec(event)  %Be careful of the last segment
                % direct signal front & tail
                front=eventMark(2*event-1)+(i-1)*(10*fSample);
                if i<(tenSec(event)-1)
                    tail=eventMark(2*event-1)+i*(10*fSample)-1;
                else 
