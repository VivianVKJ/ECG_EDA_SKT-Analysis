...
ECGSavePath='F:\Graduation Project\A Study Record\Week 16\ECG_SKT\';
EDASavePath='F:\Graduation Project\A Study Record\Week 16\ECG_SKT\';
TDFormat='.2d.mat';
VRFormat='.vr.mat';
...
for people=4:5
   ...
   %% -------------------------- 2D ENVIRONMENT  -------------------------- 
    ...
    FileName=[SignalPath,Name,TDFormat];
    load (FileName);  
    ...
    RecordECG_SKT(1:1,1:videos)=Order(5:5,2:(videos+1));  %[PeopleIndex,2d(2:10)]
    RecordECG_SKT(2:2,1:videos)=tenSec(1:videos);
    %  2.2 EDA
    RecordEDA=zeros(2,videos); %video & index
    RecordEDA(1,1:videos)=Order(5:5,2:videos+1);
